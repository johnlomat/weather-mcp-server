    ...
    <Card elevationLevel={1}>
      <Card.Header>
        <Card.ChipGroup>
          ...
          {product.is_in_stock ? (
            <Chip variant="neutral">In Stock</Chip>
          ) : (
            <Chip variant="error">Out of Stock</Chip>
          )}
        </Card.ChipGroup>
      </Card.Header>
      <Card.Body>
        ...
        <div style={{ fontSize: '24px', fontWeight: 600, textAlign: 'center', margin: '12px 0' }}>
          {product.on_sale ? (
            <>
              <span style={{ textDecoration: 'line-through', color: '#999', fontSize: '16px', marginRight: '8px' }}>
                {regularPrice}
              </span>
        ...
        <Card.Description>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {product.average_rating !== '0' && (
              <Badge variant="neutral">⭐ {product.average_rating} ({product.review_count})</Badge>
            )}
            {product.categories.slice(0, 2).map((cat) => (
              <Badge key={cat.name} variant="neutral">{cat.name}</Badge>
            ))}
          </div>
        </Card.Description>
      </Card.Body>
